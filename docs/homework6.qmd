...
my_spec <- list(
  ...
  mark = "bar",# Define the mark type (bar chart)
  height = 450,
  width = 600,
  encoding = list(
    x = list(field = "Date", type = "nominal", title = "Date"),
    y = list(
      field = "Rate Value", 
      type = "quantitative", 
      title = "Rate Value (%)",
      scale = list(domain = c(0, 100), nice = TRUE, step = 5)  # Set y-axis to have steps of 5
    ),
    color = list(field = "Rate Type", type = "nominal", title = "Rate Type")
  )
)

# Render the chart using vegawidget
my_spec |> as_vegaspec()
